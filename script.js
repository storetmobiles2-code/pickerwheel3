    ...
    showPrizeModal(prize) {
        ...
        if (prize.icon) {
            if (this.prizeEmoji.tagName === 'IMG') {
                ...
                // Add error handling - fallback to emoji if image fails
                prizeIcon.onerror = () => {
                    // Create a new div element for emoji fallback
                    const emojiDiv = document.createElement('div');
                    emojiDiv.id = 'prizeEmoji';
                    emojiDiv.className = 'prize-emoji';
                    emojiDiv.textContent = prize.emoji;
                    
                    // Replace the failed image with emoji div
                    prizeIcon.parentNode.replaceChild(emojiDiv, prizeIcon);
                    this.prizeEmoji = emojiDiv;
                };
                
                // Replace the emoji element with the image
                this.prizeEmoji.parentNode.replaceChild(prizeIcon, this.prizeEmoji);
                this.prizeEmoji = prizeIcon;
            }
        } else {
            // Fallback to emoji - ensure we have a div element
            if (this.prizeEmoji.tagName !== 'DIV') {
                const emojiDiv = document.createElement('div');
                emojiDiv.id = 'prizeEmoji';
                emojiDiv.className = 'prize-emoji';
                emojiDiv.textContent = prize.emoji;
                
                this.prizeEmoji.parentNode.replaceChild(emojiDiv, this.prizeEmoji);
                this.prizeEmoji = emojiDiv;
            } else {
                this.prizeEmoji.textContent = prize.emoji;
            }
        }
